...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Create the map once per theme/basemap URL set. Do not call `setStyle` on a live map:
  // theme swaps leave async tile loads that can run while `map.style` is undefined (MapLibre
  // vector_tile_source reads `map.style.projection`), which throws "reading 'projection'".
  //
  // Defer construction with requestAnimationFrame so React Strict Mode's first mount cycle
  // is fully torn down before any map (and basemap vector tile requests) exists — otherwise
  // tile `loadTile` can await network, then run after `map.remove()` with no style.
  useEffect(() => {
    ...
    // Recreate map when theme or default basemap URLs change — avoids setStyle races.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [resolvedTheme, mapStyles.dark, mapStyles.light, projection]);
  ...
  return (
    <MapContext.Provider value={contextValue}>
      ...
      >
        ...
        {/* Wait for style so layers/markers never run against an incomplete style (avoids MapLibre projection errors). */}
        {mapInstance && isLoaded && isStyleLoaded ? children : null}
      </div>
    </MapContext.Provider>
  );
});
...
> = {
  ...
  /** Hover over an unclustered point (null when pointer leaves the point) */
...
>({
  ...
  useEffect(() => {
    ...
    const normalizeUnclusteredCoordinates = (
      e: MapLibreGL.MapMouseEvent,
    ...
    const handleMouseEnterPoint = (
      ...
      if (onPointClick || onUnclusteredPointHover) {
        map.getCanvas().style.cursor = "pointer";
      }
      if (!onUnclusteredPointHover || !e.features?.length) return;
      const feature = e.features[0];
      const coordinates = normalizeUnclusteredCoordinates(e, feature);
      onUnclusteredPointHover({
